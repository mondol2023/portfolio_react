...
check("no discontinuity across the day, midnight wrap included", maxJump < 0.002, `max step ${maxJump.toFixed(6)}`);
check("the colour ramp has no kinks at the keyframes", maxKink < 2e-6, `max kink ${maxKink.toExponential(2)}`);
